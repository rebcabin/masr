...
To produce conforming or invalid entities inline to
other code, write
...
- =defmulti=[fn:: https://clojuredocs.org/clojure.core/defmulti] :: ---
  ...
  this case, which fetches a tag value from any
  candidate hash-map. The =defmulti= dispatches to a
  ...
  as tag-fetchers for hash-maps in Clojure.[fn::
...
intents, will be invoked. It must ignore its
argument, =_=:
...
=::intent-enum= keyword. The =::intent-enum=
sub-spec is recursively checked, as one can see in
Section [[examples-of-intent]] below.
...
Entities have shorter expressions:
...
The sugar for ttypes is too permissive. On balance,
we accept this fact. See
[[https://github.com/rebcabin/masr/issues/3][Issue 3 on MASR's GitHub repo]] for discussion. This
is subject to change.
The following tests illustrate.

The fully-sugared specs for =Integer=, =Real=,
=Complex=, and =Logical= take two keyword arguments,
=:kind= and =dimensions==:

\vskip 0.26cm
#+begin_src clojure :eval never
  (is (s/valid? ::asr/asr-ttype-head
                (Integer :kind 1, :dimensions [])))
  (is (s/valid? ::asr/asr-ttype-head
                (Integer :kind 4, :dimensions [])))
  (is (s/valid? ::asr/asr-ttype-head
                (Real :kind 8, :dimensions [])))
  (is (s/valid? ::asr/asr-ttype-head (Real)))
#+end_src

Incorrect values for the keyword arguments are invalid:

\vskip 0.26cm
#+begin_src clojure :eval never
  (is (not (s/valid? ::asr/asr-ttype-head
                     (Integer :kind 42))))
  (is (not (s/valid? ::asr/asr-ttype-head
                     (Integer :dimensions 'fubar))))
#+end_src

\newpage
The keyword arguments default to  =:kind 4= and =dimensions []=:
...
Missing keywords are a **PITFALL**. The keyword
values default to nil, which are replaced with the
defaults =:kind 4= and =dimensions []=.
