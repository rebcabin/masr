...
(deftest TranslationUnit-very-small-function-test
  (testing "testing a very small function"
    (is (s/valid? ::asr/SymbolTable
                  (asr-eval '(SymbolTable 6 {}))))
    (is (s/valid? ::asr/FunctionType
                  (asr-eval '(FunctionType
                              []
                              ()
                              Source
                              Implementation
    ...
    (is (s/valid? ::asr/dependencies
                  (asr-eval (vec (for [e '[test_fn137]] `'~e)))))
    (is (s/valid? ::asr/body
                  (asr-eval '[(SubroutineCall
                               7 test_fn1
                               ()
                               []
                               ()
...
(deftest slurp-test
  (let [hand-written-quoted-e2e0267
    ...
    #_(test-unit -test_numpy_03-6dd742e) ;; ! too big !
    #_(test-unit -test_numpy_04-3376b7a) ;; ! too big !
    (test-unit -test_pow-6f6a69d)
    (test-unit -tuple1-ce358d9)
    #_(test-unit -vec_01-9b22f33)        ;; ! too big !
