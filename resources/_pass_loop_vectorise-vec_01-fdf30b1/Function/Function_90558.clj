(ns masr.resources._pass_loop_vectorise-vec_01-fdf30b1.Function.Function_90558)


#:masr.specs{:term :masr.specs/symbol,
             :asr-symbol-head
             #:masr.specs{:symbol-head :masr.specs/Function,
                          :body
                          ["Assignment_90303"
                           #:masr.specs{:term :masr.specs/stmt,
                                        :asr-stmt-head
                                        #:masr.specs{:stmt-head
                                                     :masr.specs/Return}}],
                          :dependencies ["_lfortran_dasin"],
                          :deterministic false,
                          :function-signature "FunctionType_90126",
                          :side-effect-free false,
                          :access
                          #:masr.specs{:term :masr.specs/access,
                                       :access-enum "Public"},
                          :return-var? "Var_89289",
                          :param* ["Var_89280"],
                          :SymbolTable
                          #:masr.specs{:term :masr.specs/SymbolTable,
                                       :symtab-id 37,
                                       :hash-map
                                       {:_lpython_return_variable
                                        "Variable_89853",
                                        :x "Variable_89856"}},
                          :function-name
                          "__lpython_overloaded_0__arcsin"}}

