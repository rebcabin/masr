(ns masr.resources._pass_loop_vectorise-vec_01-fdf30b1.Function.Function_90495)


#:masr.specs{:term :masr.specs/symbol,
             :asr-symbol-head
             #:masr.specs{:symbol-head :masr.specs/Function,
                          :body
                          ["Assignment_90261"
                           #:masr.specs{:term :masr.specs/stmt,
                                        :asr-stmt-head
                                        #:masr.specs{:stmt-head
                                                     :masr.specs/Return}}],
                          :dependencies ["_lfortran_slog"],
                          :deterministic false,
                          :function-signature "FunctionType_90063",
                          :side-effect-free false,
                          :access
                          #:masr.specs{:term :masr.specs/access,
                                       :access-enum "Public"},
                          :return-var? "Var_89061",
                          :param* ["Var_89052"],
                          :SymbolTable
                          #:masr.specs{:term :masr.specs/SymbolTable,
                                       :symtab-id 35,
                                       :hash-map
                                       {:_lpython_return_variable
                                        "Variable_89721",
                                        :x "Variable_89724"}},
                          :function-name
                          "__lpython_overloaded_1__log2"}}

